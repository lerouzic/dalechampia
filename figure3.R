...
pdf("figure3.pdf", width=8, height=8, useDingbats = FALSE)
layout(cbind(1:2,3:4))
par(mar=c(3, 4.2, 3, 1))
plot.data.recenter(
	recenter(tovar, G=tovar.mat$G, Gv=tovar.mat$Gv, P=tovar.mat$P, N=64, Np=12, target="mu.x", normalization="updown"),
	main="Tovar", ylab=expression("log GA (mm"^2*")"), xlab="", ylim=c(-0.4,0.4), CI.factor=CI.factor)
par(mar=c(5, 4.2, 1, 1))
plot.data.recenter(
...
par(mar=c(3, 4.2, 3, 1))
plot.data.recenter(
	recenter(tulum, G=tulum.mat$G, Gv=tulum.mat$Gv, P=tulum.mat$P, N=64, Np=12, target="mu.x", normalization="updown", G0.boost=TRUE),
	main="Tulum", ylab="", xlab="", ylim=c(-0.4,0.4), CI.factor=CI.factor)
par(mar=c(5, 4.2, 1, 1))
plot.data.recenter(
	recenter(tulum, G=tulum.mat$G, Gv=tulum.mat$Gv, P=tulum.mat$P, N=64, Np=12, target="mu.y", normalization="updown", G0.boost=TRUE),
