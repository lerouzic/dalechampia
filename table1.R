...
data.tab1.col1 <- list(tovar$G.VCV[,1], tovar.G.cor, tovar$P.VCV[,1], tovar.P.cor, tulum$G.VCV[,1], tulum.G.cor, tulum$P.VCV[,1], tulum.P.cor)
data.tab1.col2 <- list(tovar$G.VCV[,2], tovar$G.VCV[,4], tovar$P.VCV[,2], tovar$P.VCV[,4], tulum$G.VCV[,2], tulum$G.VCV[,4], tulum$P.VCV[,2], tulum$P.VCV[,4])
...
tab1.col1 <- paste0(
	...
	"; ", 
	format(round(rep(c(mf,1), 4)*sapply(data.tab1.col1, function(x) HPDinterval(x)[2]), digits=dd), nsmall=dd),
	")")
...
table1 <- data.frame(' '=as.character(c("Tovar", "", "", "", "Tulum", "", "", "")), '  '=as.character(c("G-matrix", "", "P-matrix", "", "G-matrix", "", "P-matrix", "")), '   '=rep(c("GA","UBA"), 4), 'Gland area (GA)'=tab1.col1, 'Upper bract area (UBA)'=tab1.col2, check.names=FALSE)

write.table(table1, file="table1.txt", quote=FALSE, sep="\t", row.names=FALSE)
